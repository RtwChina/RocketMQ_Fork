...
public class CommitLog {
    ...
    public DispatchRequest checkMessageAndReturnSize(java.nio.ByteBuffer byteBuffer, final boolean checkCRC,
        ...
        // 读取一个完整的Message,返回一个DispatchRequest对象
        try {
            // 1 TOTAL SIZE
    ...
    public PutMessageResult putMessage(final MessageExtBrokerInner msg) {
        ...
        // 获取最近一个CommitLog文件的内存映射文件（零拷贝）,即MappedFile
        MappedFile unlockMappedFile = null;
        MappedFile mappedFile = this.mappedFileQueue.getLastMappedFile();

        // PutMessage会有多个工作者线程在并行处理，需要上锁，可配置可重入锁和自旋锁
        putMessageLock.lock(); //spin or ReentrantLock ,depending on store config
        try {
            long beginLockTimestamp = this.defaultMessageStore.getSystemClock().now();
            this.beginTimeInLock = beginLockTimestamp;

            // Here settings are stored timestamp, in order to ensure an orderly 需要拿到锁再设置时间
            // global
            ...
            // 内存映射 mapperFile 这是真正的写入消息啦，还会传入写入消息的回调
            // 把Broker内部的这个Message刷新到MappedFile的内存，还没有刷盘
    ...
    public void handleDiskFlush(AppendMessageResult result, PutMessageResult putMessageResult, MessageExt messageExt) {
        ...
        if (FlushDiskType.SYNC_FLUSH == this.defaultMessageStore.getMessageStoreConfig().getFlushDiskType()) {
            ...
            if (messageExt.isWaitStoreMsgOK()) {
                // 封装成GroupCommitRequest，且塞入到GroupCommitService服务线程中
                ...
                // 虽然Broker配置成同步刷盘，但客户端不关心是否已经刷盘成功
                // 同步刷盘策略退化成了异步刷盘策略
                service.wakeup();
            }
        }
    ...
    class DefaultAppendMessageCallback implements AppendMessageCallback {
        ...
        public AppendMessageResult doAppend(final long fileFromOffset, final ByteBuffer byteBuffer, final int maxBlank,
            ...
            // PHY OFFSET
            //fileFromOffset :一个CommitLog文件(对应一个mappedFile)对应的偏移量（文件名就对应偏移量）
            // byteBuffer.position()：当前MappedFile(对应一个CommitLog)的写位置
            // 消息的绝对物理位置
            long wroteOffset = fileFromOffset + byteBuffer.position();

            this.resetByteBuffer(hostHolder, 8);
            // 通过broker的存储地址和消息绝对物理地址算出唯一的msgId
            ...
            if (null == queueOffset) {
                queueOffset = 0L;
                // queueOffset表示某个TOPIC的某个Queue的消息数量，视为后面重建consumerQueue做准备
            ...
            // Determines whether there is sufficient free space  commitlog是否还有空余空间
