...
//        1、首先对PullResult进行处理，主要完成如下3件事：1）对消息体解码成一条条消息 2）执行消息过滤 3）执行回调钩子
...
//        1) Step1:根据MQ的Broker信息获取查找Broker信息，封装成FindBrokerResult。
        /*
        根据BrokerName、BorkerId从MQClientInstance中获取Broker地址，在整个RocketMQ Broker的部署结构中，
...
//        然后通过网络去 拉取具体的消息，也就是消息体 中的数据。具体数据拉取逻辑，在重点分析消息存储时重点去研究。
        if (findBrokerResult != null) {
            ...
            // Step2 : 若消息为类过滤模式，则需要获取Broker上的FilterServer地址，从FilterServer上拉取消息，否则从Broker上拉取消息。
            String brokerAddr = findBrokerResult.getBrokerAddr();
            if (PullSysFlag.hasClassFilterFlag(sysFlagInner)) {
                brokerAddr = computPullFromWhichFilterServer(mq.getTopic(), brokerAddr);
            }

            // Step3 : 拉取请求都组装完毕后向MQClientInsatance拉取消息
