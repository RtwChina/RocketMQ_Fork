 ...
 * </p>
 */
// 推消息消费者  broker往消费端推送数据，其内部实现了流控，消费位置上报等等。
public class DefaultMQPushConsumer extends ClientConfig implements MQPushConsumer {
    ...
    /**
     * Message listener  客户端消费消息的实现类
     */
    private MessageListener messageListener;

    /**
     * Offset Storage offset存储实现，分为本地存储或远程存储
     */
    private OffsetStore offsetStore;

    /**
     * Minimum consumer thread number  线程池自动调整
     */
    private int consumeThreadMin = 20;

    /**
     * Max consumer thread number 线程池自动调整
     */
    ...
    /**
     * Message pull Interval   //消息拉取时间间隔，默认为0，即拉完一次立马拉第二次，单位毫秒
     */
    private long pullInterval = 0;

    /**
     * Batch consumption size  1//并发消费时，一次消费消息的数量，默认为1，假如修改为50，此时若有100条消息，那么会创建两个线程，每个线程分配50条消息。
     */
    private int consumeMessageBatchMaxSize = 1;

    /**
     * Batch pull size //消息拉取一次的数量
     */
